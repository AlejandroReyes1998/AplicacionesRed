...
def recvall(sock,deSerialize=None):	
    ...
    while (True):           
        ...
        if (len(part) < BUFF_SIZE): 
            break                     
    if(deSerialize):
        return jsonpickle.decode(tiempot)
    else: 
        return tiempot
...
def iniciarjuegoconcepto(dificultad,lista,vsauce,name):
	...
	instruccion_server = jsonpickle.encode(["inicio"])	
	s.send(instruccion_server.encode())
	...
	try:
		...
		sys.exit(0)
	instruccion_server = jsonpickle.encode(["fin",name,dificultades[dificultad],str(found),"Conceptos"])
	s.send(instruccion_server.encode())
	s.shutdown(socket.SHUT_WR)
	tiempot = recvall(s,True) 
	s.close()
	print("Jugador: "+name)
	print("Numero de palabras encontradas: "+str(found))
	print("Tiempo tardado: "+ str(tiempot))
...
def iniciarjuegoanagrama(lista,name):
	...
	try:
		...
		sys.exit(0)
	instruccion_server = jsonpickle.encode(["fin",name,"-",str(found),"Anagramas"])
	s.send(instruccion_server.encode())
	s.shutdown(socket.SHUT_WR)
	tiempot = recvall(s,True)
	s.close()
...
def main():
	...
	if s == 1:
		...
		if y==0:#ANIMALES
			...
			try:
				...
				return False
			instruccion_server = jsonpickle.encode(["animales"])	
			s.send(instruccion_server.encode())
			s.shutdown(socket.SHUT_WR)
		...
		if y==1:#FRUTAS
			...
			try:
				...
				return False
			instruccion_server = jsonpickle.encode(["frutas"])	
			s.send(instruccion_server.encode())
			s.shutdown(socket.SHUT_WR)
		...
		if y==2:#AUTOS
			...
			try:
				...
				return False
			instruccion_server = jsonpickle.encode(["autos"])	
			s.send(instruccion_server.encode())
			s.shutdown(socket.SHUT_WR)
		...
		if y==3:#PAISES
			...
			try:
				...
				return False
			instruccion_server = jsonpickle.encode(["paises"])	
			s.send(instruccion_server.encode())
			s.shutdown(socket.SHUT_WR)
			paises = recvall(s,True)
			#NO REPETITIONS
			while i != 14:
	...
	if s==2:
		...
		try:
			...
			return False
		instruccion_server = jsonpickle.encode(["palabras"])	
		s.send(instruccion_server.encode())
		s.shutdown(socket.SHUT_WR)
		...
		try:
			...
			return False
		instruccion_server = jsonpickle.encode(["puntajes"])	
		s.send(instruccion_server.encode())
		s.shutdown(socket.SHUT_WR)
